...
const Chart = ({corona : {histories, new_update}, page : {home, global, country, city}}) => {
  ...
  const historiesGlobalChart = (
    <>
      <Line                
        ...
        className={styles.historiesGlobalChart}
        data={{
          labels: historiesGlobal.map(({date}) => date),
          datasets :[
            {
              data : historiesGlobal.map(({confirmed}) => confirmed),
              label : "Bị nhiễm",
              borderColor: "#5a5a5a",
              fill : true
            },
            {
              data : historiesGlobal.map(({recovered}) => recovered),
              label : "Hồi phục",
            ...
            {
              data : historiesGlobal.map(({deaths}) => deaths),
              label : "Tử vong",
    ...
    <React.Fragment>
      <Pie
        ...
        data = {{
          ...
          datasets : [{
            data : [new_update.totalActive -  new_update.totalCritical, new_update.totalCritical],
          ...
          <div class={styles.cards}>
            <div className={styles.card}>
              ...
              <div className={styles.card_content}>
                <div className={clsx(styles.side,styles.front)} ref={refFrontActive}>
                  <div className={styles.general}>
                    <div className={styles.general_num}><CountUp start={0} end={new_update.totalActive} duration={2.5} separator=",">{new_update.totalActive}</CountUp></div>
                    <div className={styles.general_sub}>Số bệnh nhân hiện tại</div>
                  </div>
                  <div className={styles.details}>
                    <div className={styles.mild_condition}>
                      <div className={styles.detail_mild_num}>
                        <CountUp start={0} end={new_update.totalActive- new_update.totalCritical} duration={2.5} separator=",">{new_update.totalActive}</CountUp> ({(((new_update.totalActive- new_update.totalCritical)/new_update.totalActive)*100).toFixed(0)}%)</div>
                      <div className={styles.detail_mild_sub}>Tình trạng bình thường</div>
                    </div>
                    <div className={styles.critical_condition}>
                    <div className={styles.detail_crit_num}>
                      <CountUp start={0} end={new_update.totalCritical} duration={2.5} separator=",">{new_update.totalActive}</CountUp> ({((( new_update.totalCritical)/new_update.totalActive)*100).toFixed(0)}%)</div>
                      <div className={styles.detail_crit_sub}>Nguy kịch</div>
                    </div>
                  </div>                               
                </div>                
                <div className={clsx(styles.side,styles.back,styles.pieChartActiveCase)} ref={refBackActive}>
                  {pieChartActiveCase}
                </div>
              </div>    
              <div className={styles.card_action}>
                <a href="#!" className={styles.link} onClick={toggleGraph1}>{tgGraph1 ? "Xem số liệu" : "Xem biểu đồ"}</a>
              </div>                 
            </div>    
            ...
            <div className={styles.card}>
              ...
              <div className={styles.card_content}>
                <div className={clsx(styles.side,styles.front)} ref={refFrontClosed}>
                  <div className={styles.general}>
                    <div className={styles.general_num}>
                      <CountUp start={0} end={closedCase} duration={2.5} separator=",">{closedCase}</CountUp>
                    </div>
                    <div className={styles.general_sub}>Số ca không còn ghi nhận</div>
                  </div>
                  <div className={styles.details}>
                    <div className={styles.mild_condition}>
                      <div className={styles.detail_recovered_num}>
                      <CountUp start={0} end={recovered} duration={2.5} separator=",">{recovered}</CountUp> ({((recovered/closedCase )*100).toFixed(0)}%)
                      </div>
                      <div className={styles.detail_recovered_sub}>Số ca hồi phục</div>
                    </div>
                    <div className={styles.critical_condition}>
                    <div className={styles.detail_deaths_num}>
                      <CountUp start={0} end={closedCase-recovered} duration={2.5} separator=",">{closedCase-recovered}</CountUp> ({ (((closedCase-recovered)/closedCase )*100).toFixed(0)}%)
                    </div>
                      <div className={styles.detail_deaths_sub}>Số ca tử vong</div>
                    </div>
                  </div>                               
                </div>
                
                <div className={clsx(styles.side,styles.back,styles.pieChartActiveCase)} ref={refBackClosed}>
                  {pieChartClosedCase}
                </div>
              </div>    
              <div className={styles.card_action}>
                <a href="#!" className={styles.link} onClick={toggleGraph2}>{tgGraph2 ? "Xem số liệu" : "Xem biểu đồ"}</a>
              </div>                 
            </div>          
          </div>
          <div>
            <h3>DÒNG THỜI GIAN</h3>
            <div className={styles.global_chart}>
            {historiesGlobalChart}
            </div>
          </div>
